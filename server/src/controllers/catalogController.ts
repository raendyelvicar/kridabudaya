...
import { createCatalogAssets } from '../services/catalogAssetService';
const formidable = require('formidable');

export const getAll = async (req: Request, res: Response): Promise<void> => {
  try {
    const search = req.query.search?.toString();
    const clients = await getAllCatalog(search);
    res.json(clients);
  } catch (error) {
    sendErrorResponse(
      res,
...
export const create = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const catalog = await createCatalog(model);
    ...
    await Promise.all(
      files.map(async (file) => {
        try {
          ...
          const catalogAssetModel: CatalogAssetsEntity = {
            file_id: uploadedFile.fileid,
            catalog_id: catalog.id,
          };
          await createCatalogAssets(catalogAssetModel);
        } catch (error) {
...
export const update = async (req: Request, res: Response): Promise<void> => {
  try {
    const reqBody: CatalogEntity = req.body;
    const catalogid: number = parseInt(req.params.id);
    ...
    const updateResult = await updateCatalog(catalogid, reqBody);
...
export const deleteData = async (
  ...
  try {
    ...
    const deleteResult = await deleteCatalog(catalogid);
